...
[ -z "$REPO_URI" ] && REPO_URI='https://github.com/michaellincn/spf13-vim.git'
...
setup_local_mode() {
    ...
    if [ "$1" -eq '1' ]; then
        ...
        touch "$target_path/.gvimrc.local"
        ...
        lnif "$source_path/.gvimrc.local"  "$target_path/.gvimrc.local"
...
setup_local_mode "$local_maintainer" \
                "$APP_PATH" \
                "$HOME"
